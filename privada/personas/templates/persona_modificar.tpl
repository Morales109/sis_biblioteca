...
<body>
	...
	<form action="persona_modificar1.php" method="post" name="formu">
		...
		<p>
			<input type="text" name="telef" size="15" placeholder="Telefono" onkeyup="this.value=this.value.toUpperCase()" value="{$r.telef}">
		</p>
		<p>
			<input type="hidden" name="accion" value="">
			<input type="hidden" name="id_persona" value="{$r.id_persona}">
			<input type="button" name="Aceptar" onclick="validar();"class="boton2">
			<input type="button" name="Cancelar" onclick="javascript:location.href='personas.php';"class="boton2">
		</p>
		{/foreach}
